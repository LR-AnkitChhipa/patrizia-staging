...
export default async (context, inject) => {
  ...
  let api = {}
  try {
    api = await Prismic.api('https://patrizia-living-cities.cdn.prismic.io/api/v2', Object.assign({}, options,  {}))
  } catch (error) {
    console.error(error)
  ...
  let prismic = new Vue({
    computed: {
      ...
      apiEndpoint() {
        return 'https://patrizia-living-cities.cdn.prismic.io/api/v2'
      },
      predicates() {
  ...
  if (process.client) {
    window.onNuxtReady && window.onNuxtReady(() => {
      ...
      script.src = '//static.cdn.prismic.io/prismic.min.js?repo=patrizia-living-cities&new=true'
  ...
  if (process.client && process.static && route.path !== '/preview') {
    ...
    const repo = 'patrizia-living-cities'
