      ...
      <b-row>
        <b-col cols="12" md="8" lg="6">
          <div id="register" class="password-reset-form">
            <div>
              <p class="subline subline--with-dash" v-if="slice.primary.forgotten_password_form_subline">
                {{ slice.primary.forgotten_password_form_subline }}
              </p>
              <h2 v-if="slice.primary.forgotten_password_form_title" class="register__title">{{ slice.primary.forgotten_password_form_title }}</h2>
              <prismic-rich-text :field="slice.primary.forgotten_password_form_content" />
            </div>
                ...
                <form @submit.prevent="onSubmit">
                  ...
                  <div class="form-row submit">
                    <div class="col-md-12 mb-3">
                      ...
                      >
                        <form-button button-size="regular" button-style="primary primary--style1" label="Submit"/>
                      </b-overlay>
                    </div>
                  </div>
                </form>
...
export default {
  ...
  methods: {
    ...
    sucessAlert() {
      this.alert.msg = 'Your password request has been submitted. You will receive an email with instructions on how to reset your password.'
    ...
    onSubmit() {
      ...
      this.$refs.form.validate().then((success) => {
        ...
        axios.get(process.env.VUE_APP_BACKEND_URL + process.env.VUE_APP_SANCTUM_ENDPOINT + '/csrf-cookie', {withCredentials: true}).then((response) => {
          ...
          axios.post(process.env.VUE_APP_BACKEND_URL + '/forgot-password', this.formData, {withCredentials: true})
