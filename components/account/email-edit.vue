  ...
  <div class="nameupdate">
    <div class="nameupdate__content">
      <div class="nameupdate__content__form-details">
        <p class="mb-5">
          Current email address: <span>{{ formData.email }}</span>
          <br><br>
          ipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua
        </p>
              ...
              <form @submit.prevent="onUpdate">
                <div class="form-row flex-wrap">
                  <div class="item col item--three mb-3">
                    <ValidationProvider v-slot="{ classes, errors }" :bails="false" name="email" rules="required|email">
                      <label :class="classes" for="email">NEW EMAIL ADDRESS</label>
                      <input
                        id="email"
...
export default {
  ...
  computed: {
    ...
    newEmail: {
      set (email) {
        this.$store.commit('SET_USER_PROPERTY', { email })
      },
      get () {
